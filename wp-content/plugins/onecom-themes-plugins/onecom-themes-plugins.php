...
if( ! defined( 'ONECOM_WP_VERSION' ) ) {
	define( 'ONECOM_WP_VERSION', '0.5.8' );
}
...
if( ! function_exists( 'register_one_core_resources' ) ) {
	function register_one_core_resources( $hook ) {
		 ...
		 **/
		do_action( 'limit_enqueue_resources', $handle = OC_PLUGIN_DOMAIN, $hook, $type = 'style' );
		do_action( 'limit_enqueue_resources', $handle = OC_PLUGIN_DOMAIN, $hook, $type = 'script' );
...
if( ! function_exists( 'one_core_admin' ) ) {
	function one_core_admin() {
		...
		add_menu_page(
			...
			$menu_title = __( 'One.com', OC_PLUGIN_DOMAIN ),
