...
class API {
    ...
    public function save_template( $args = array() ){
        ...
        $saved_template = Plugin::$instance->templates_manager->save_template( $args );
    ...
    public static function get_categories(){
        ...
        if( is_wp_error( $data ) ) {
            $error_message = $data->get_error_message();
            Helper::send_error(__( "Something Went Wrong: " . $error_message, 'templately' ));
        }
    ...
    private function get_items( $type = 'items' ) {
        ...
        $query = sprintf(
            ...
            $per_page,
            esc_attr( $platform ),
            ( $type === 'packs' ) ? '' : 'dependencies{ id, name, slug, icon },'
        );
    ...
    private function cloud_push( $name, $data, $item_type = 'elementor', $args = array() ){
        ...
        if( is_wp_error( $response ) ) {
            ...
            Helper::send_error( $error_message );
        }
    ...
    public function template_export( $id = null ){
        ...
        return Plugin::$instance->templates_manager->export_template( array(
            'source' => 'local', 
            'template_id' => $id
        ) );
    }
    ...
    protected function check_dependency(){
        ...
        if( ! empty( $dependencies ) && is_array( $dependencies ) ) {
            foreach( $dependencies as $dependency ) {
                if( is_null( $dependency->slug ) ) {
                    continue;
                }
                if( ! \is_plugin_active( $dependency->slug ) ) {
                    $not_active_plugin_list[] = $dependency->name;
                }
            }
        }
    ...
    protected function get_item(){
        ...
        $query = '';
        if( $type === 'block' ) {
            $query = '{ items( slug: "'. $slug .'" ) { data { slug, rating, type, downloads, dependencies{ id, icon, name, slug }, live_url, name, id, retina_ready, documentation_ready, features, favourite_count, thumbnail, price, author{ display_name, name, profile_photo, joined }, tags{ name }, category{ name, id } } } }';
        }
        if( $type === 'pack' ) { 
            $query = '{ 
                packs( slug: "'. $slug .'" ) { 
                    ...
                    items {
                        slug, type, live_url, name, downloads, rating, id, dependencies{ id, icon, name, slug }, retina_ready, documentation_ready, features, favourite_count, thumbnail, price, tags{ name }, category{ name, id } } }
                } 
            }';
        }

        if( $type === 'page' ) {
            $query = '{ pages( slug: "'. $slug .'" ) { data { slug, type, rating, downloads, dependencies{ id, icon, name, slug }, live_url, name, id, retina_ready, documentation_ready, features, favourite_count, thumbnail, price, author{ display_name, name, joined, profile_photo }, tags{ name }, category{ name, id } } } }';
        }
    ...
    public function create_page(){
        ...
        if ( $inserted_ID && ! is_wp_error( $inserted_ID ) ) {
			return array(
                ...
                'elementor_edit_link' => $platform === 'elementor' ? Plugin::$instance->documents->get( $inserted_ID )->get_edit_url() : false,
    ...
    public function login_user(){
        ...
        if( $withApi ) {
            ...
            $query = \sprintf(
                'mutation{ connectWithApiKey( api_key : "%s", site_url : "%s", ip: "%s" ){ status, message, user { id, first_name, last_name, name, profile_photo, api_key, email, favourites { id, type }, joined, plan, plan_expire_at }}}',
                $api_key,
                \home_url('/'),
                self::get_ip()
            );
            ...
            $query = \sprintf(
                'mutation{ connect( email : "%s", password : "%s", site_url : "%s", ip: "%s" ){ status, message, user { id, first_name, last_name, name, plan, plan_expire_at, profile_photo, api_key, email, favourites { id, type }, joined, my_cloud { limit, usages, name, files{ data { id, name, thumbnail }, current_page, total_page }}}}}',
                $email,
                $password,
                \home_url('/'),
                self::get_ip()
    ...
    protected function clouds_sync(){
        ...
        if( ! empty( $api_key ) ) {
            ...
            $query = \sprintf(
                '{ myCloud( api_key: "%s", cloud_files_page: %s, cloud_files_per_page: %s, file_type: "%s"%s ) { limit, usages, name, files {  data{ id, name, thumbnail, file_type, created_at }, current_page, total_page } } }',
                $api_key,
                $pagenum,
                $per_page,
                $file_type,
    ...
    protected function get_workspace(){
        ...
        $response = Query::get( $query );
        if( is_wp_error( $response ) ) {
            Helper::send_error( __( 'Something went wrong loading WorkSpace. Try again reloading.', 'templately' ) );
        }
    ...
    protected function create_workspace(){
        ...
        $response = Query::get( $query );
        if( is_wp_error( $response ) ) {
            Helper::send_error( __( 'Something went wrong regarding create WorkSpace. Try again.', 'templately' ) );
        }
    ...
    protected function edit_workspace(){
        ...
        $response = Query::get( $query );
        if( is_wp_error( $response ) ) {
            Helper::send_error( __( 'Something went wrong regarding edit WorkSpace. Try again.', 'templately' ) );
        }
    ...
    protected function create_or_edit_workspace( $edit = false ){
        ...
        $response = Query::get( $query );
        if( is_wp_error( $response ) ) {
            Helper::send_error( __( "Something went wrong regarding $action WorkSpace. Try again.", 'templately' ) );
        }
    ...
    protected function delete_workspace(){
        ...
        $response = Query::get( $query );
        if( is_wp_error( $response ) ) {
            Helper::send_error( __( 'Something went wrong regarding delete WorkSpace. Try again.', 'templately' ) );
        }
    ...
    protected function copy_or_move(){
        ...
        $response = Query::get( $query );
        if( is_wp_error( $response ) ) {
            Helper::send_error( __( "Something went wrong regarding $action to WorkSpace. Try again.", 'templately' ) );
        }
    ...
    protected function workspace_details() {
        ...
        $response = Query::get( $query );
        if( is_wp_error( $response ) ) {
            Helper::send_error( __( "Something went wrong regarding loading your WorkSpace. Try again.", 'templately' ) );
        }
    ...
    protected function cloud_usage_limit() {
        ...
        $response = Query::get( $query );
        if( is_wp_error( $response ) ) {
            Helper::send_error( __( "Something went wrong regarding collecting your usage limit. Try again.", 'templately' ) );
        }
    ...
    protected function add_files_to_workspace() {
        ...
        $response = Query::get( $query );
        if( is_wp_error( $response ) ) {
            Helper::send_error( __( "Something went wrong regarding adding files to your Workspace. Try again.", 'templately' ) );
        }
